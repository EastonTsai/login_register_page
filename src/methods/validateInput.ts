export const validateAccount = (account: string) => {
  if (account.trim().length < 1) { return '帳號不能空白' }
  const regex = /(?=.*[a-z])(?=.*[A-Z])/
...
export const validateEmail = (email: string) => {
  ...
  if (!regex.test(email)) { return 'email 格式錯誤' }
}
export const validatePassword = (password: string) => {
  if (password.trim().length < 1) { return '密碼不能空白' }
  if (password.trim().length < 6) { return '密碼字數不足' }
  const regex = /(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])/
  if (!regex.test(password)) { return '密碼格式錯誤' }
}
export const validateCheckPassword = (password: string, checkPassword: string) => {
  if (checkPassword.trim().length < 1) { return '密碼不能空白' }
  if (password !== checkPassword) { return '確認密碼不相符' }
}
type resultType = {
  accountError?: string,
  emailError?: string,
